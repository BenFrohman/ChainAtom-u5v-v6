...
License: Apache-2.0. See LICENSE / NOTICE if present; otherwise this header is the license notice.

Atomic block of the locked sextic

    F = (x0^5 x3 + x3^6) + (x1^5 x4 + x4^6) + (x2^5 x5 + x5^6).

Sister lab (full Jacobian / zeta / Lean): [BenFrohman/SingularityLab](https://github.com/BenFrohman/SingularityLab).
...
Two variables. Isolated plane-curve singularity at the origin.
    ...
    A = [[5,1],[0,6]]
    |Aut(W)| = |det A| = 30
    μ(W) = 25     (Jacobian dimension; not 30)
